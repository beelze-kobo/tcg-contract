...
contract TCGmarketplace is ERC1155Receiver, Ownable, ReentrancyGuard {
    struct Escrow {
        address nftOwner;
        address nftContract;
        uint256 nftID;
    ...
    event EscrowCreated(uint256 indexed escrowId, address indexed nftOwner, address indexed nftContract, uint256 nftID, uint256 nftAmount, uint256 animeAmount);
    event EscrowRemoved(uint256 indexed escrowId);
    event NFTPurchased(uint256 indexed escrowId, address indexed buyer);
    ...
    function createEscrow(
        ...
        require(IERC1155(_nftContract).balanceOf(msg.sender, _nftID) >= _nftAmount, "You don't own enough of this NFT");
        ...
        emit EscrowCreated(escrows.length - 1, msg.sender, _nftContract, _nftID, _nftAmount, _animeAmountInWei);
    }
    ...
    function removeEscrow(uint256 i) public {
        ...
        emit EscrowRemoved(i);
    }

    /// @notice Buyer sends exact ANIME (native) to purchase
    function buyWithAnime(uint256 i) external payable nonReentrant {
        Escrow memory e = escrows[i];
        require(msg.value >= e.animeAmount, "Not enough ANIME sent");
        require(msg.sender != e.nftOwner, "Seller cannot buy their own NFT");
        ...
        emit NFTPurchased(i, msg.sender);
    }
